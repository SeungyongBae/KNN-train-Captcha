...
for char in chars:
    ...
    if input >= 48 and input <= 57:
        name = str(input - 48)
        file_count = len(next(os.walk('answer/training_data/' + name + '/'))[2])

        cv2.imwrite('answer/training_data/' + str(input - 48) + '/' +
        ...
        cv2.imwrite('answer/training_data/' + name + '/' +
        str(file_count + 1) + '.png', resized)